@props(['name', 'class' => 'w-5 h-5', 'stroke' => 2])
{{-- Line icons from Lucide (lucide.dev, ISC licence), inlined so the app
     has no runtime icon dependency. Replaces the emoji the UI used to lean
     on. Emoji render differently on every OS and make the app look
     auto-generated. Add an icon: copy the inner markup of its SVG from
     lucide-static into the map below. --}}
@php
    $icons = [
...
@endphp
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="{{ $stroke }}" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" {{ $attributes->merge(['class' => $class]) }}>{!! $icons[$name] ?? '' !!}</svg>
